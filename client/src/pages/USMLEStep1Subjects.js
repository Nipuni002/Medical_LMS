...
function USMLEStep1Subjects({
  step = 'STEP_1',
  heading = 'USMLE STEP 1 Subjects',
  description = 'Choose a subject tab to focus your STEP 1 preparation.',
  fallbackSubjects = []
}) {
  ...
  useEffect(() => {
    const fetchStep1Subjects = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/usmle-subjects?step=${step}`);
        if (!response.ok) {
        ...
        const mappedSubjects = data.map((subject, index) => ({
          ...
          color: subject.color ? `${subject.color}aa` : 'rgba(37, 99, 235, 0.64)'
        })).sort((a, b) => {
        ...
        console.error('Failed to fetch USMLE subjects:', error);
        setSubjects(fallbackSubjects);
      } finally {
    ...
    fetchStep1Subjects();
  }, [step, fallbackSubjects]);

  const openSubjectContent = (subjectId) => {
    navigate(`/exams/usmle/theory/${subjectId}`);
  };
    ...
    <div className="usmle-step1-page">
      ...
      <main className="usmle-step1-content">
        ...
        <section className="usmle-step1-panel">
          {loadingSubjects ? (
            <div className="subject-tabs" role="group" aria-label="USMLE subject buttons">
              <p className="subject-empty-state">Loading subjects...</p>
            </div>
          ) : subjects.length === 0 ? (
            <div className="subject-tabs" role="group" aria-label="USMLE subject buttons">
              <p className="subject-empty-state">No subjects available yet. Admin can add subjects from the USMLE admin panel.</p>
            </div>
          ) : (
            <div className="subject-groups" aria-label="USMLE subjects by weightage">
              {WEIGHTAGE_ORDER.map((weightage) => {
                ...
                return (
                  <section key={weightage} className="weightage-section">
                    ...
                    <div className="subject-tabs" role="group" aria-label={`${weightage} USMLE subjects`}>
                      {list.map((subject) => (
                        <button
                          key={subject.id}
                          type="button"
...
export default USMLEStep1Subjects;