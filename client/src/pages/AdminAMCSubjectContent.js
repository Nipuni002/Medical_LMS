...
function AdminAMCSubjectContent() {
  ...
  const handleDeleteTopicConfirm = async () => {
    if (deleteTopicIndex === null || !formData.topics[deleteTopicIndex]) {
      showNotification('Topic not found', 'error');
      return;
    }

    const newTopics = formData.topics.filter((_, idx) => idx !== deleteTopicIndex);
    const saved = await persistContent(newTopics, 'Topic deleted successfully!');
    if (!saved) return;
  ...
  return (
    <div className="admin-theory-subject-content-page">
      {notification && (
      ...
      <div className="admin-header">
        <div className="header-left">
          <button onClick={backToStepPage} className="back-btn">
            ← Back to AMC Subjects
          </button>
          <h1>Manage AMC Subject Topics</h1>
          {selectedSubject && (
            <div className="subject-meta">
              <span className="subject-badge">{selectedSubject.name}</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="content-card">
          ...
          <div className="topics-section">
            <div className="topics-header">
              <h3>Topics List ({formData.topics.length})</h3>
            ...
            {formData.topics.length === 0 ? (
              <div className="no-topics-placeholder">
                <p>No topics added yet. Click "+ Add New Topic" to create your first topic.</p>
              </div>
            ) : (
              <div className="topics-list-admin">
                {formData.topics.map((topic, index) => (
                  <div key={`${topic.title}-${index}`} className="topic-card-admin">
                    <div className="topic-card-header">
                      <span className="topic-order">Topic {index + 1}</span>
                    ...
                    <div className="topic-summary">
                      ...
                      <p><strong>Videos:</strong> {topic.videoLinks?.length || 0} links</p>
                      <p>
