...
function NEXTStepSubjects({
  step = 'STEP_1',
  heading = 'NExT Step 1 Subjects',
  description = 'Choose a subject to open NExT theory content.'
}) {
  ...
  useEffect(() => {
    const fetchStepSubjects = async () => {
      try {
        ...
        console.error('Failed to fetch NExT subjects:', error);
        setSubjects([]);
      } finally {
  ...
  const openSubjectContent = (subjectId) => {
    navigate(`/exams/next/theory/${subjectId}`);
  };
    ...
    <div className="usmle-step1-page">
      ...
      <main className="usmle-step1-content">
        ...
        <section className="usmle-step1-panel">
          {loadingSubjects ? (
            <div className="subject-tabs" role="group" aria-label="NExT subject buttons">
              <p className="subject-empty-state">Loading subjects...</p>
            </div>
          ) : subjects.length === 0 ? (
            <div className="subject-tabs" role="group" aria-label="NExT subject buttons">
              <p className="subject-empty-state">No subjects available yet. Admin can add NExT subjects from the admin panel.</p>
            </div>
          ) : (
            <div className="subject-groups" aria-label="NExT subjects by weightage">
              {WEIGHTAGE_ORDER.map((weightage) => {
                ...
                return (
                  <section key={weightage} className="weightage-section">
                    ...
                    <div className="subject-tabs" role="group" aria-label={`${weightage} NExT subjects`}>
                      {list.map((subject) => (
                        <button
                          key={subject.id}
                          type="button"
...
export default NEXTStepSubjects;
