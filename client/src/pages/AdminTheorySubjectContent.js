...
function AdminTheorySubjectContent() {
  ...
  const fetchInitialData = async () => {
    if (!subjectId) {
      navigate('/admin/subjects-content?category=plab');
      return;
    }
    ...
    try {
      ...
      if (!Array.isArray(subjects)) {
        showNotification(subjects?.message || 'Error loading subjects', 'error');
        setLoading(false);
        return;
      }
  ...
  const handleAddVideoLinkField = (isEdit = false) => {
    if (isEdit) {
      setEditTopic(prev => ({ ...prev, videoLinks: [...prev.videoLinks, ''] }));
    } else {
      setNewTopic(prev => ({ ...prev, videoLinks: [...prev.videoLinks, ''] }));
    }
  };

  const handleRemoveVideoLinkField = (index, isEdit = false) => {
    if (isEdit) {
      setEditTopic(prev => ({
        ...prev,
  ...
  const handlePdfFileUpload = (e, isEdit = false) => {
    ...
    reader.onload = (event) => {
      ...
      if (isEdit) {
        setEditTopic(prev => ({ ...prev, pdfUrl: dataUrl }));
      } else {
        setNewTopic(prev => ({ ...prev, pdfUrl: dataUrl }));
      }
      showNotification('PDF file attached successfully!', 'success');
    };
  ...
  const handleAddTopicConfirm = async () => {
    ...
    const cleanedVideoLinks = (newTopic.videoLinks || [])
      .map(l => (l || '').trim())
      .filter(Boolean);
    const primaryVideo = cleanedVideoLinks[0] || newTopic.videoLink || '';
    ...
    const newTopics = [
      ...formData.topics,
      {
        ...
        videoLink: primaryVideo,
        videoLinks: cleanedVideoLinks,
  ...
  const openEditTopicModal = (index) => {
    ...
    const existingLinks = topic.videoLinks && topic.videoLinks.length > 0
      ? topic.videoLinks
      : topic.videoLink ? [topic.videoLink] : [''];
  ...
  const handleEditTopicConfirm = async () => {
    ...
    const cleanedVideoLinks = (editTopic.videoLinks || [])
      .map(l => (l || '').trim())
      .filter(Boolean);
    const primaryVideo = cleanedVideoLinks[0] || editTopic.videoLink || '';
    ...
    return (
      <div className="admin-theory-content">
        <div className="admin-header">
          ...
          <button onClick={() => navigate('/admin/subjects-content?category=plab')} className="back-button">
            ← Back to Subjects
          </button>
        </div>
    ...
    <div className="admin-theory-content">
      ...
      <div className="content-form-container">
        <div className="subject-selector">
          <div className="admin-subject-bank-header">
            ...
            <p>{selectedSubject.weightage} | Add or update this subject content below.</p>
          </div>
        </div>
      ...
      {showAddTopicModal && (
        <div className="modal-overlay" onClick={handleAddTopicCancel}>
          <div className="modal-dialog topic-editor-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder="e.g., Ischaemic Heart Disease & ACS"
              ...
              <div className="form-group">
                <label>Topic Description (Displayed first at top of topic page)</label>
                <textarea
                  rows={3}
                  value={newTopic.description}
                  onChange={(e) => setNewTopic(prev => ({ ...prev, description: e.target.value }))}
                  placeholder="Overview and key clinical summary for this topic..."
                  className="topic-input"
                />
              </div>

              {/* SECTION 2: VIDEOS (MULTIPLE - DISPLAYED HORIZONTALLY 3 PER LINE) */}
              <div className="form-group">
                <div className="options-header">
                  <label>Videos (Displayed horizontally 3 per row on topic page)</label>
                  <button
                    type="button"
                    className="add-opt-btn"
                    onClick={() => handleAddVideoLinkField(false)}
                  >
                    + Add Video Link
                  </button>
                </div>
                <div className="admin-video-inputs">
                  {(newTopic.videoLinks || ['']).map((vLink, idx) => (
                    <div key={idx} className="video-input-row">
                      <span className="opt-prefix">🎥 {idx + 1}</span>
                      <input
                        type="url"
                        value={vLink}
                        onChange={(e) => {
                        ...
                        className="topic-input"
                      />
                      {newTopic.videoLinks.length > 1 && (
                        <button
                          type="button"
                          className="remove-opt-btn"
                          onClick={() => handleRemoveVideoLinkField(idx, false)}
                        >
                          ×
                        </button>
                      )}
            ...
            <div className="modal-actions">
              <button onClick={handleAddTopicCancel} className="cancel-btn">
                Cancel
              </button>
      ...
      {showEditTopicModal && (
        <div className="modal-overlay" onClick={handleEditTopicCancel}>
          <div className="modal-dialog topic-editor-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  onChange={(e) => setEditTopic(prev => ({ ...prev, description: e.target.value }))}
                  placeholder="Overview and key clinical summary for this topic..."
                  className="topic-input"
                />
              </div>
              ...
              <div className="form-group">
                <div className="options-header">
                  ...
                  <button
                    type="button"
                    className="add-opt-btn"
                    onClick={() => handleAddVideoLinkField(true)}
                  >
                    + Add Video Link
                  </button>
                </div>
                <div className="admin-video-inputs">
                  {(editTopic.videoLinks || ['']).map((vLink, idx) => (
                    <div key={idx} className="video-input-row">
                      <span className="opt-prefix">🎥 {idx + 1}</span>
                      <input
                        type="url"
                        ...
                        className="topic-input"
                      />
                      {editTopic.videoLinks.length > 1 && (
                        <button
                          type="button"
                          className="remove-opt-btn"
                          onClick={() => handleRemoveVideoLinkField(idx, true)}
                        >
                          ×
                        </button>
                      )}
            ...
            <div className="modal-actions">
              <button onClick={handleEditTopicCancel} className="cancel-btn">
                Cancel
              </button>
      ...
      {showDeleteTopicModal && (
        <div className="modal-overlay" onClick={handleDeleteTopicCancel}>
          <div className="modal-dialog delete-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              <button onClick={handleDeleteTopicCancel} className="cancel-btn">
                Cancel
              </button>
...
export default AdminTheorySubjectContent;
