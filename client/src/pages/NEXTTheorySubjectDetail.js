...
function NEXTTheorySubjectDetail() {
  ...
  const handleDownloadPDF = () => {
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <meta charset="UTF-8">
          <title>${topic?.title || 'Topic'} - ${content?.title || 'NExT Theory Content'}</title>
          <style>
            @page {
              margin: 2cm;
              size: A4;
        ...
        <body>
          <div class="document-header">
            <div class="subject-title">${content?.title || 'NExT Theory Content'}</div>
          ...
          <div class="document-footer">
            <div class="footer-brand">Medical LMS - NExT Theory Content</div>
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/api/next-theory-content/subject/${subjectId}`);
        ...
        console.error('Error fetching NExT content:', fetchError);
        ...
        <div className="detail-container">
          ...
          >
            <div className="header-content">
              ...
              <h1 className="subject-title">{content?.title || 'NExT Theory Content'}</h1>
            </div>
          </div>
                      ...
                      <div className="topic-notes-section">
                        <div className="notes-header-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '15px' }}>
                          ...
                          <div className="notes-action-buttons">
                            <button
                              onClick={() => navigate(`/questions/${subjectId}?exam=NEXT&topicId=${activeTopic._id}`)}
...
export default NEXTTheorySubjectDetail;
