...
function AdminUSMLEStep1Pretest() {
  ...
  const [formData, setFormData] = useState({
    slug: 'usmle-step1-pretest',
    title: 'USMLE Step 1 Pre-Test',
    instructions:
      '7 blocks, each up to 40 questions. 60 minutes per block. Total testing time 7 hours with 45 minutes break allowance.',
  ...
  const fetchTest = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/plab-tests/admin/usmle-step1-pretest', {
        headers: {
  ...
  const validateQuestionForm = () => {
    ...
    if (questionForm.blockNumber < 1 || questionForm.blockNumber > 7) {
      showNotification('Block number must be between 1 and 7.', 'error');
      return false;
    }
  ...
  const handleOpenPublishDialog = () => {
    setConfirmDialog({
      isOpen: true,
      type: 'publish',
      title: 'Publish USMLE Step 1 Pre-Test',
  ...
  if (loading) {
    return <div className="admin-plab-tests-loading">Loading USMLE Step 1 pretest...</div>;
  }
  ...
  return (
    <div className="admin-plab-tests-page">
      <div className="admin-tests-header">
        <div>
          <h1>Manage USMLE Step 1 Pre-Test</h1>
          <p className="admin-tests-subtitle">Add questions, answers, explanations, and assign each question to one of 7 blocks.</p>
        </div>
      ...
      <section className="admin-tests-overview" aria-label="USMLE pretest overview">
        ...
        <article className="overview-tile">
          ...
          <p>{Object.values(blockCounts).filter((count) => count > 0).length}/7</p>
        </article>
        <article className="overview-tile">
      ...
      <section className="admin-tests-card questions-list-card">
        ...
        <p className="admin-tests-list-note">Questions are grouped by block for the 7-block exam flow.</p>
...
export default AdminUSMLEStep1Pretest;
