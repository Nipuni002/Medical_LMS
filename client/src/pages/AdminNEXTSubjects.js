...
const STEP_OPTIONS = [
  { value: 'STEP_1', label: 'Step 1 (Theory)', defaultColor: '#1d4ed8' },
  { value: 'STEP_2', label: 'Step 2 (Clinical)', defaultColor: '#2563eb' }
];
...
function AdminNEXTSubjects() {
  ...
  const fetchSubjects = async () => {
    try {
      ...
      console.error('Error fetching NExT subjects:', error);
      setSubjects([]);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return <div className="admin-usmle-loading">Loading NExT subjects...</div>;
  }
  ...
  return (
    <div className="admin-usmle-subjects">
      <header className="usmle-admin-header">
        <div>
          <h1>NExT Subjects Management</h1>
          <p>Manage subject lists for NExT Step 1 and Step 2.</p>
        </div>

        <div className="usmle-admin-actions">
          <button className="btn-secondary" onClick={() => navigate('/admin/dashboard?section=next-admin')}>
            Back to NExT Admin
          </button>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-card" onClick={(event) => event.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <label htmlFor="step">NExT Step</label>
...
export default AdminNEXTSubjects;
