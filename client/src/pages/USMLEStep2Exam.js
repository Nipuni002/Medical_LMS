...
const USMLE_STEP2_PRETEST_PROGRESS_KEY_PREFIX = 'usmle-step2-pretest-progress';
const USMLE_STEP2_PRETEST_COMPLETED_KEY_PREFIX = 'usmle-step2-pretest-completed';
const USMLE_STEP2_PRETEST_MODE_KEY = 'usmle-step2-pretest-exam-mode';

const getStep2ProgressKey = (mode) => `${USMLE_STEP2_PRETEST_PROGRESS_KEY_PREFIX}-${mode}`;
const getStep2CompletedKey = (mode) => `${USMLE_STEP2_PRETEST_COMPLETED_KEY_PREFIX}-${mode}`;

function USMLEStep2Exam() {
  ...
  const [examMode, setExamMode] = useState(() => {
    ...
    const savedMode = localStorage.getItem(USMLE_STEP2_PRETEST_MODE_KEY);
    ...
    const fetchTest = async () => {
      try {
        ...
        if (!data.success || !data.data) {
          setError('USMLE Step 2 CK pre-test is not available right now.');
          setLoading(false);
          return;
        }
  ...
  useEffect(() => {
    localStorage.removeItem('usmle-step2-pretest-progress');
    localStorage.removeItem('usmle-step2-pretest-completed');

    const routeExamMode = location.state?.examMode;
    const savedExamMode = routeExamMode || localStorage.getItem(USMLE_STEP2_PRETEST_MODE_KEY);
  ...
  useEffect(() => {
    const completedFlag = localStorage.getItem(getStep2CompletedKey(examMode)) === 'true';
    ...
    const savedProgressRaw = localStorage.getItem(getStep2ProgressKey(examMode));
    if (!savedProgressRaw) {
      setPhase('instructions');
      setCurrentBlock(1);
    ...
    try {
      ...
      const savedCompletedBlocksFree = Array.isArray(savedProgress.completedBlocksFree)
        ? savedProgress.completedBlocksFree
      ...
      const savedCompletedBlocksProfessional = Array.isArray(savedProgress.completedBlocksProfessional)
        ? savedProgress.completedBlocksProfessional
          .map((value) => Number(value))
          .filter((value) => value >= 1 && value <= BLOCK_COUNT)
        : [];
      ...
      localStorage.removeItem(getStep2ProgressKey(examMode));
    }
  }, [examMode]);

  useEffect(() => {
    localStorage.setItem(USMLE_STEP2_PRETEST_MODE_KEY, examMode);
  }, [examMode]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(getStep2ProgressKey(examMode), JSON.stringify(progressPayload));
  }, [
    loading,
    hasCompletedAttempt,
    test,
  ...
  }, [completedBlocksProfessional, isFreeMode]);

  const startSpecificBlock = (blockNumber) => {
    if (blockNumber < 1 || blockNumber > BLOCK_COUNT || (!isFreeMode && blockNumber > highestUnlockedBlock)) {
      return;
    }
  ...
  const finishExam = () => {
    localStorage.setItem(getStep2CompletedKey(examMode), 'true');
    localStorage.removeItem(getStep2ProgressKey(examMode));
    setHasCompletedAttempt(true);

    navigate('/exams/usmle/step2-pretest/review', {
      state: {
        test,
        answers
      }
  ...
  const handleReattemptAllBlocks = () => {
    localStorage.removeItem(getStep2CompletedKey(examMode));
    localStorage.removeItem(getStep2ProgressKey(examMode));
  ...
  const handleReattemptCurrentBlock = () => {
    setAnswers((prev) => {
      ...
      blockQuestions.forEach((_question, indexInBlock) => {
        const globalIndex = getGlobalIndex(currentBlock, indexInBlock);
        if (Object.prototype.hasOwnProperty.call(next, globalIndex)) {
    ...
    if (isFreeMode) {
      setCompletedBlocksFree((prev) => prev.filter((block) => block !== currentBlock));
    } else {
      setCompletedBlocksProfessional((prev) => prev.filter((block) => block !== currentBlock));
    }
  ...
  if (loading) {
    return (
      <div className="plab1-test-page">
        <Header />
        <div className="plab1-test-loading">Loading USMLE Step 2 CK pre-test...</div>
        <Footer />
      </div>
    );
  }
    ...
    return (
      <div className="plab1-test-page">
        ...
        <main className="plab1-test-wrapper">
          <h1>{test.title || 'USMLE Step 2 CK Pre-Test'}</h1>
          <section className="step1-mode-selector" aria-label="Choose Step 2 CK exam mode">
            <div className="step1-mode-copy">
              <p className="step1-mode-kicker">Choose Exam Mode</p>
              <h2>Professional Way or Free Blocks</h2>
              <p>
                Modes are separate. You can switch modes and attempt blocks independently.
              </p>
            </div>
          ...
          <section className="exam-instruction-card">
            ...
            <p className="plab1-test-instructions">
              You have already completed this {examMode === 'free' ? 'Free Blocks' : 'Professional Way'} attempt.
            </p>
          ...
          <>
            <section className="step1-mode-selector" aria-label="Choose Step 2 CK exam mode">
              <div className="step1-mode-copy">
                ...
                <h2>Professional Way or Free Blocks</h2>
                <p>
                  Professional Way unlocks one block at a time. Free Blocks opens every block so you can move freely.
                </p>
              </div>
            ...
            <section className="usmle-blocks-board" aria-label="USMLE Step 2 CK blocks">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((block) => {
                const isCompleted = isFreeMode
                  ? completedBlocksFree.includes(block)
                  : completedBlocksProfessional.includes(block);
                const isUnlocked = isFreeMode || block <= highestUnlockedBlock;
                const isActive = phase !== 'instructions' && block === currentBlock;
                const blockStatus = isCompleted ? 'Completed' : isFreeMode ? 'Open' : isUnlocked ? 'Unlocked' : 'Locked';
                const blockDescription = isCompleted
                  ? 'Review complete for this block.'
                  : isFreeMode
                    ? 'All blocks are open in Free Blocks mode.'
                    : isUnlocked
                    ? isActive
            ...
            <section className="exam-instruction-card">
              ...
              <p className="plab1-test-instructions">
                {isFreeMode
                  ? 'Free Blocks mode lets you open any block in the set.'
        ...
        {phase === 'instructions' ? null : phase === 'break' ? (
          ...
          <section className="question-card question-list-card">
            ...
            <div className="question-actions">
              {isFreeMode ? (
                <>
                  <button type="button" className="nav-btn secondary-btn" onClick={handleReattemptCurrentBlock}>
                    Reattempt Block
                  </button>
                  <button type="button" className="nav-btn secondary-btn" onClick={handleBackToBlocksPage}>
                    Back To Block Page
                  </button>
                  <button type="button" className="nav-btn end-test-btn" onClick={handleContinueNextBlock}>
                ...
                <>
                  {currentBlock < BLOCK_COUNT && totalBreakRemainingSeconds > 0 && (
                    <button type="button" className="nav-btn secondary-btn" onClick={handleGoToBreak}>
      ...
      {showNavigationLockModal && (
        <div className="exam-nav-lock-overlay" role="presentation" onClick={() => setShowNavigationLockModal(false)}>
          ...
          >
            ...
            <p>You cannot leave this exam now. Click "Go To Break" first.</p>
            <button
              type="button"
