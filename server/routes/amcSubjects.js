...
router.get('/', async (req, res) => {
  try {
    ...
    const subjects = await AmcSubject.find(query)
    ...
    console.error('Error fetching AMC subjects:', err.message);
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const existingSubject = await AmcSubject.findOne({
      step,
    ...
    console.error('Error creating AMC subject:', err.message);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.put('/:id', protect, async (req, res) => {
  try {
    ...
    const subject = await AmcSubject.findById(req.params.id);
    ...
    const duplicate = await AmcSubject.findOne({
      _id: { $ne: subject._id },
      step: targetStep,
    ...
    console.error('Error updating AMC subject:', err.message);
    res.status(500).json({ message: 'Server error' });
  }
});
...
router.delete('/:id', protect, async (req, res) => {
  try {
    ...
    await AmcSubject.findByIdAndDelete(req.params.id);
    res.json({ message: 'Subject deleted successfully' });
  } catch (err) {
    console.error('Error deleting AMC subject:', err.message);
    res.status(500).json({ message: 'Server error' });
  }
});
